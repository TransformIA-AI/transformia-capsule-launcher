...
import { buildActivationPackFileManifest, buildV09FreezeCandidateSummary, type SaaSActivationPack } from './saas-activation-pack-handoff';

function assertInside(root: string, target: string): void {
  const rel = relative(root, target);
  if (rel.startsWith('..') || rel === '..' || rel.includes(`..${sep}`) || resolve(target) !== target) throw new Error('blocked_path_traversal');
}
function publicJson(value: unknown): string { return `${JSON.stringify(value, null, 2)}\n`; }
export function buildActivationPackWritableFiles(pack: SaaSActivationPack): Record<string, string> {
  return {
...
export function writeSaaSActivationPack(pack: SaaSActivationPack, outputRoot: string): string[] {
